...
This project uses the HB100 Doppler Module to track speeds of different objects and displays it on the 7 segment display of the Nexys A7 FPGA board. To run the code simply clone the repo, open the xpr file, make sure top module is selected as top, and generate bitstream. After you have flashed the bitstream the Nexys A7 FPGA, build the circuit provided in the README system architecture section. You will need op-amps (I used MCP6002, but any op-amp works), resistors, and capacitors (electrolytic and ceramic). Connect the output of the circuit to JD1 of the FPGA. Also connect gnd of circuit to gnd of pmod JD. You can use the 3D printed chasis I have provided, though you will need double-sided tape for the HB100 to mount it and the FPGA board to make it stay still. 
